   ...
   //implements MouseListener
{
   ...
   // returns a randomly chosen blank Square
   public Square pickASquare()
   {
      Square sq = null;
      
      do
      {
         int i = (int) (Math.random()*boardSize); // 0-2
         int j = (int) (Math.random()*boardSize); // 0-2
         
         if ( !theSquares[i][j].isOccupied() ) { sq = theSquares[i][j]; }
      }
      while ( sq == null );
