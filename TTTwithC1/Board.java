   ...
   //implements MouseListener
{
   ...
   // listOfEmpties returns a list of Squares that are empty
   public LinkedList<Square> listOfEmpties()
   {
   ...
   public Square pickEmpty()
   {
       LinkedList<Square> list = listOfEmpties();
       int size = list.size();
       int choice = (int)(Math.random()*size);
       Square s = list.get(choice);
       return s;
   }
   ...
   public Player myTurn()
   {
      ...
      mine = thePlayer[whoseTurn()];
      
      return mine;
   }
   
   static int getBoardSize() { return boardSize; } 
   static void setTheGame( TTT ttt ) { theGame = ttt; }
}
